# kie.ai proxy — POST /api/kie/jobs/createTask  (Vercel Python serverless function)
# Holds the kie.ai key server-side (env KIE_AI_API_KEY) and forwards the request,
# so the key is never exposed in the browser. Basic same-site Referer guard.
